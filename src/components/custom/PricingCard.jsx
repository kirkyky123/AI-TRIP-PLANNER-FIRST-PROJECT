...
const TIER_BORDERS = {
  Business: "border-[#fb7272]",
  Pro: "border-[#34f0ac]",
};

function PricingCard({ title, price, features, customPrice = false }) {
  const outline = TIER_BORDERS[title] ?? "border-[#f9e42b]";
    ...
    <div className="relative z-10 items-center overflow-hidden rounded-xl p-[2.5px] hover:scale-110 h-full transform transition duration-300">
      <div className={`relative z-20 flex rounded-[0.60rem] bg-gradient-to-b from-white to-orange-200 dark:from-black dark:to-black p-[4px] h-full ${outline} border-2`}>
        <div className=" rounded-[0.55rem] p-[2px] flex-grow">
          <div className="rounded-lg p-6 shadow-lg border-4 border-transparent flex flex-col justify-between h-full relative">
            {title === "Pro" && (
            ...
            <div className="text-black dark:text-white">
              ...
              <p className="text-xl sm:text-3xl lg:text-xl xl:text-3xl font-bold mb-4">
                ...
                {price === "$9.99/month" ? (
                  <span className="bg-gradient-to-l to-[#38da97] via-red-600 from-blue-400 text-transparent bg-clip-text">
                    {price}
                  </span>
                ) : (
                  price
            ...
            <Button className="w-full mt-6 dark:bg-black border-2 dark:border-white font-bold dark:hover:bg-white dark:hover:text-black hover:bg-black hover:text-white">
